...
function a_theme_core_setup()
{
	...
	add_image_size('rectangle-founder', 700, 700, true);
...
function a_theme_core_header_scripts()
{
	if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
		...
		if ( is_file($vendorHeadStyle) ) {
			wp_enqueue_style('vendors-style-head', get_template_directory_uri() . '/dist/css/vendors-head.min.css', array(), THEME_VERSION."1.5");
        }
        wp_enqueue_style('theme-style', get_template_directory_uri() . '/dist/css/theme.min.css', array(), THEME_VERSION."1.5");

        //Video Files
        wp_enqueue_style( 'mbYTPlater', get_template_directory_uri() . '/assets/styles/jquery.mbYTplayer.css', array(), '1.0.1');
        //Including jQuery mbYTPlater plugin
        wp_register_script('mb-YTPlater', get_template_directory_uri() . '/assets/scripts/jquery.mb.YTplayer.js', array('jquery'), THEME_VERSION, false);
        wp_enqueue_script('mb-YTPlater');

        //Adding Animate.css Styles
        wp_enqueue_style('theme-style-animate', get_template_directory_uri() . '/assets/style/animate.css', array(), THEME_VERSION);
